...
var hui = (function(selector, undefined) {
	...
	var getDoms = function(selector) {
		...
		if(selectorArray.length < 2) {
			...
			for(var i = 1; i < selectorArray.length; i++) {
				lastDoms = lastDoms.find(selectorArray[i]);
			}
			return lastDoms;
		}
		return addFuns(null);
	...
	var hcExtends = {
		...
		unfold: function(height, text, color) {
			if(this.length < 1) {
				return
			}
			if(!height) {
			...
			buttonDom.innerHTML = '<span class="hui-icons hui-icons-down2" style="color:' + color + '">' + text + '</span>';
			hui(buttonDom).appendTo(this);

			hui('#hui-unfold').click(function() {
				var _span = hui(this).find('span');
				if(_span.hasClass('hui-icons-down2')) {
					hui(this).parent().css({
						height: 'auto'
					});
					_span.html('收起全文');
					_span.removeClass('hui-icons-down2');
					_span.addClass('hui-icons-up2');
					return;
				}
				hui(this).parent().css({
					height: height + 'px'
				...
				_span.html('展开全文');
				_span.removeClass('hui-icons-up2');
				_span.addClass('hui-icons-down2');
	...
	huibase.open = function(winName, styles, isClose, extras) {
		...
		plus.webview.show(w, 'zoom-fade-out',300);
		return w;
	};
	...
	huibase.exitApp = function() {
		var selfW = plus.webview.getLaunchWebview();
		if(huibackNum < 1) {
			hui.toast('再按一次退出程序');
			huibackNum++;
			setTimeout(function() {
	...
	// 增加控制css样式 2017年11月22日18:25:14 by liaciton
	huibase.maskShow = function(cssText) {
		...
		if(!huibase.mask) {
			...
			if (cssText) {
				huibase.mask.style.cssText = cssText;
			}
	...
	huibase.createDom = function(domTag) {
	...
	huibase.immersedStatusbar = function() {
		hui.plusReady(function() {
			...
			if(isImmersedStatusbar) {
				...
				hui('.hui-header').eq(0).css({
					'paddingTop': StatusbarHeight + 'px'
				});
				hui('#hui-back').css({
					top: StatusbarHeight + 'px'
				});
				hui('#hui-header-menu').css({
					top: StatusbarHeight + 'px'
				});
