...
默认不可滑动,只能点击 2017-11-29 By Liaction
*/
hui.tab = function(selector, isCanSwipe) {
	var tabLists = hui(selector);
	for(var i = 0; i < tabLists.length; i++) {
		var tabRun = new huiTabListBase(tabLists.dom[i], isCanSwipe);
	}
}
huiTabListBase = function(dom, isCanSwipe) {
	if (typeof(isCanSwipe)=="undefined") {
		isCanSwipe = false;
	}
	this.tab = hui(dom);
	this.tabBody = this.tab.find('.hui-tab-body').eq(0);
	...
	this.tab.find('.hui-tab-body').css({
		'height': this.height + 'px'
	});
	this.tabListTitles = this.tab.find('.hui-tab-title').eq(0).find('div');
	this.tabListTitles.css({
		width: (100 / this.itemSize) + "%"
	});
	...
	this.swpieMove = 0;
	if(isCanSwipe) {
		this.tabBody.touchMove(function(e) {
			_selfObj.moveScale = e.x / _selfObj.width * -1;
			_selfObj.swpieMove = _selfObj.index + _selfObj.moveScale * 1.5;
			if(_selfObj.swpieMove < 0 || (_selfObj.swpieMove > _selfObj.itemSize - 1)) {
				return false;
			}
			var i = 0;
		...
		this.tabBody.touchEnd(function(e) {
			if(_selfObj.swpieMove < 0 || (_selfObj.swpieMove > _selfObj.itemSize - 1)) {
				return false;
			}
			_selfObj.index = Math.round(_selfObj.swpieMove);
			_selfObj.change();
		});
	}
